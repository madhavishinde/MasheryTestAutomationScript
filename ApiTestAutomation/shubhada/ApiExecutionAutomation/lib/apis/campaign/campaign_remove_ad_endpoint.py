...
class campaign_remove_ad_endpoint:
        ...
        def remove_ad_operation(self):
                ...
                try:
                        if not variables.campaign_id:
                                ...
                                response = requests.post('%s/%s/%s/remove_ad' %(variables.base_url, variables.url_path, variables.campaign_id), verify=False, headers=headers, data=json.dumps(variables.payload))
